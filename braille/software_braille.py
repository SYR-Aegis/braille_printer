...
def change(in_str):
    ...
    def change_english(change_list):  # 만약 처음이라면 끝이라면 등등 고려해야 되네
        ...
        for keyword in change_list:
            ...
            elif keyword == "." or keyword == "?" or keyword == "!" or keyword == "," or keyword == "-" or keyword == "*":
    ...
    def jaum(keyword):
        ...
        nonlocal return_list
        #딕셔너리를 사용해서 해보자 
        char = {'ㄱ' : 0b000100 ,'ㄴ' : 0b100100 ,'ㄷ' : 0b010100 ,'ㄹ' : 0b000010 , 'ㅁ' : 0b100010 ,'ㅂ' : 0b000110, 'ㅅ' : 0b000001 ,'ㅇ' : 0b011011, 'ㅈ' : 0b000101 ,'ㅊ' : 0b000011 , 'ㅋ' : 0b110100 ,'ㅌ' : 0b110010, 'ㅍ' : 0b100110 ,'ㅎ' : 0b010110}

        jaum_string = char[keyword]  
        #print(jaum_string)
        destination = jaum_string 
        return_list.append(destination)
        return
    ...
    def find_hangul(print_list):
        ...
        for keyword in print_list:  # 만약 이 예외처리 조건에 부합하지 않는다면 분리시키고 각 각 함수로 넣는다.
         #   print(keyword)
          #  print(type(keyword))
            if int(ord(keyword)) >= 0x3131 and int(ord(keyword)) <= 0x314E:
    #            print("Suc")
                jaum(keyword)
                continue
                #자음 루틴
    ...
    def check_letter(check_list):  # 첫 글자가 한글이나 아니냐 파악 루틴
        ...
        # #printcheck_list[0])
        #print(check_list[0])
        if check >= 0x3131 and check <= 0x314E:
            return 1
    ...
    # 띄어쓰기를 위한 index# 아래것도 함수화 처리해야하나.... 
    index = 0
    string = in_str
    list_string = []
    ...
    for i in range(0, routine_value): #띄어쓰기를 기준으로함 
        ...
        if check == 2:
            ...
            if i < routine_value - 1:  # 한글과 숫자를...
                ...
                if ord(letter[i+1][0]) >= 0x3131 and ord(letter[i+1][0]) <= 0x314E:
                    destination = end_en 
                    return_list.append(destination)
                ...
                #if ord(letter[i+1][0]) >= 0x21 and ord(letter[i+1][0]) <= 0x3F:
                 #   destination = end_en
                  #  return_list.append(destination)
...
###############################################################################################
#나중엔 파일 받을 때 파일 이름을 . 으로 split해서 파싱한 다음 이걸 change()함수에 넘겨야 겠다.)
###############################################################################################
#chage()에서 얻은 리턴값을 한번 더 리턴하자 
#밑에 함수에서 문자열을 파싱한뒤 확장자를 분석한다. 
...
def change_lab(c_string):
    list_give = []
    check_extension = c_string.split('.')
    
    if check_extension[1] == "txt":
        f = open(c_string, "r",-1,"utf-8")
        while True: 
            line = f.readline()
            if not line: break
            list_give += change(line)
        f.close()
        return list_give
    
    elif check_extension[1] == "pdf":
        extract_text(c_string)
        f = open("read_pdf.txt", "r",-1,"utf-8")
        while True:
            line = f.readline()
            if not line: break 
            list_give += change(line)
